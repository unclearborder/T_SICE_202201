...
\subsection{Index for the Problem}
...
For simplicity, all tasks are assumed to be performed during this one time step.
Each task for each target be performed at least one period in the entire time, and the set of $k$th performed period set is $\mathcal{L}^k_{hj}$. 
...
In this paper, we consider placing robot $m$ at time $l$ satisfying $l^{ks}_{hj} \le l \le l^{ke}_{hj}$ to a target where $\mathcal{J}_{hm}=\mathcal{J}_{m}\cap\mathcal{J}_{h}$, the common set of $\mathcal{J}_h$ and $\mathcal{J}_m$, is $\mathcal{J}_{hm}\neq \varnothing$.

\subsection{}

\subsection{Sets for the Problem}
